...
class FeatureRepository(object):
    ...
    def _async_lock_for(self, key: str) -> "asyncio.Lock":
        """Per-(loop, key) async lock; loop-scoped to avoid cross-loop reuse."""
    ...
    def load_features(
        ...
        # and the async client already coalesces via `_remote_eval_inflight`.
        ...
        # others wait on the lock and re-read the freshly cached value.
    ...
    async def load_features_async(
        ...
        # Per-key coalescing for the CDN path. The lock is held across the network `await`,
